...
const UserList = ({ setTeam, team }) => {
  const [fetchUsers, setUsers] = useState([]);
  ...
  const getAllUsers = async () => {
    ...
    try {
      ...
      const filteredUser = res.filter(user => !user?.isAdmin);
  ...
  useEffect(() => {
    if (team?.length < 1 && fetchUsers.length > 0) {
      setSelectedUsers([fetchUsers[0]]);
    } else {
      setSelectedUsers(fetchUsers.filter((user) => team.includes(user._id)));
    }
  }, [team, fetchUsers]);
